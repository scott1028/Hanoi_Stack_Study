// 參考：http://tw.knowledge.yahoo.com/question/question?qid=1507100703097
// 盒內塔遞迴移動盤子範例

// function(盤子數, 來源柱編號, 緩衝柱編號, 目標柱編號)
var move=function(n,source_stack,temp_stack,dest_stack){
	if(n==1){
		console.log('from '+source_stack.toString()+' move to '+dest_stack.toString()+' !');
	}
	else if(n>1){
		// 將 n-1 個盤子先移到 temp 柱子上
		move(n-1,source_stack,dest_stack,temp_stack);
		// 將 source 盤最下方的一道 destination
		console.log('from '+source_stack.toString()+' move to '+dest_stack.toString()+' #');
		// 將 temp 盤子一道 destination 柱子上
